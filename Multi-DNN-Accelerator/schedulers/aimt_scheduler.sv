...
module aimt_scheduler #(
    ...
    parameter COMPUTE_BAL_THRESH  = 1000,          // balance threshold (Section 3.4.1)
    // ---------- data widths ----------
    ...
    input  logic [LAYER_ID_WIDTH-1:0]  st_prev_layer,      // single predecessor (0xFF = none)
    input  logic [CYCLE_WIDTH-1:0]     st_mem_cycles,      // cycles to load weights
    input  logic [CYCLE_WIDTH-1:0]     st_compute_cycles,  // cycles to compute layer
    input  logic [MEM_WIDTH-1:0]       st_weight_fp,       // weight footprint  (bytes)
    input  logic [MEM_WIDTH-1:0]       st_ifmap_fp,        // IFMAP  footprint  (bytes)
    input  logic [MEM_WIDTH-1:0]       st_ofmap_fp,        // OFMAP  footprint  (bytes)
    input  logic [LAYER_ID_WIDTH-1:0]  st_total_layers,    // total layers loaded
    ...
    // Scheduled CT queue (ordered for FIFO dispatch)
    ...
    // ================================================================
    logic signed [CYCLE_WIDTH:0] mem_cycle_ctr;      // cumulative memory cycles scheduled
    logic signed [CYCLE_WIDTH:0] compute_cycle_ctr;  // cumulative compute cycles completed
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (!ct_active && ct_cq_cnt > 0) begin
                ...
                if (sched_table[ct_cand].prev_layer == 8'hFF)
                    prev_done = 1'b1;
                else
